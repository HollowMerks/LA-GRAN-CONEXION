<template>
    <div class="login-wrap">
        <div class="card">
            ...
            <p v-if="horarioActual" style="margin-top:12px; text-align:center; font-weight:700;">
              Clase activa: {{ horarioActual?.materia }} | Aula: {{ horarioActual?.aulaNombre }} | Docente: {{ horarioActual?.docenteNombre }} | Bloque: {{ horarioActual?.bloque }}
            </p>
        </div>
    </div>
</template>
...
<script setup>
    ...
    onMounted(async () => {
      try {
        ...
        // Guardar idHorario temporalmente
        if (data?.idHorario) {
          localStorage.setItem('idHorarioActual', String(data.idHorario))
        }
      } catch (e) {
        // No bloquear login si el horario no está disponible
    ...
    async function requestOtp() {
        ...
        try {
            // Aquí podrías validar credenciales si aplica
            // Redirigir a la pantalla de OTP manteniendo email
            router.push({ name: 'Otp', query: { email: email.value } })
        } catch (e) {
            error.value = 'No se pudo enviar el código OTP.'
        } finally {
            loading.value = false
        }
    }
