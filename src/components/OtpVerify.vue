    ...
    <div class="otp-wrap">
        <div class="card">
            <div class="icon-container">
                <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="#ff6a00" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect>
                    <path d="M7 11V7a5 5 0 0 1 10 0v4"></path>
                </svg>
            </div>

            <h1 class="title">Verificaci&oacute;n de Seguridad</h1>
            <p class="desc">
                Hemos enviado un c&oacute;digo de 6 d&iacute;gitos a su correo o dispositivo m&oacute;vil.
            </p>
            ...
            <div v-if="horarioActual" style="margin-top:18px;">
              ...
              <p>Materia: {{ horarioActual?.materia }} | Aula: {{ horarioActual?.aulaNombre }} | Docente: {{ horarioActual?.docenteNombre }} | Bloque: {{ horarioActual?.bloque }}</p>
              ...
              <div style="margin-top:12px; display:flex; gap:8px;">
                ...
                <input v-model="observaciones" placeholder="Observaciones opcionales" />
                <button class="primary" :disabled="asistLoading" @click="registrar">{{ asistLoading ? 'Registrando…' : 'Registrar asistencia' }}</button>
              </div>

              <div style="margin-top:12px;">
                <button class="primary" @click="consultarAsistencias">Consultar asistencias</button>
              </div>

              <div v-if="asistencias" style="margin-top:12px;">
                <p>Total: {{ asistencias?.total || asistencias?.count || asistencias?.length || 0 }}</p>
                <ul>
                  <li v-for="a in (asistencias?.detalles || asistencias?.items || asistencias || [])" :key="a.idAsistencia">
...
<script setup>
    ...
    function focusNext(idx, e) {
        ...
        // Salta al siguiente input si se escribió un número
    ...
    onMounted(async () => {
      try {
        ...
        if (!storedId) {
          // Fallback: intentar obtenerlo otra vez
          ...
          // Si hay id guardado, refrescar datos del horario actual
          const data = await getHorarioActual()
          horarioActual.value = data
        }
      } catch (e) {
...
<style scoped>
    ...
    .card {
        ...
        background: #fff;
        border-radius: 14px;
        padding: 30px 22px; /* Un poco más de espacio arriba */
        box-shadow: 0 18px 40px rgba(0,0,0,0.08);
        border-top: 6px solid #ff6a00; /* Detalle estético naranja arriba */
    }
    ...
    /* Caja de los inputs OTP */
    .otp-box {
        display: flex;
        gap: 8px; /* Espacio entre cuadritos */
        justify-content: center;
        margin: 20px 0;
    }

    /* ESTILO DE LOS INPUTS (Bordes negros aquí) */
    .otp-input {
        width: 45px;
        height: 55px;
        text-align: center;
        ...
        font-weight: bold;
        border-radius: 8px;
        border: 2px solid #333; /* Borde oscuro y un poco más grueso */
        font-family: 'Poppins', system-ui;
        ...
        .otp-input:focus {
            border-color: #ff6a00; /* Se pone naranja al escribir */
            outline: none;
